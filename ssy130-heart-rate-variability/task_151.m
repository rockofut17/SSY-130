...
STD_values = zeros(4,Nr_min);
 ...
 for ff = 1:Nfiles
     ...
     x0 = data(offset+(1:NN),1); % insert some x vector depending on interval
     x_matrix = reshape(x0,7680,[]);
     ...
     for i = 1:Nr_min-1
         ...
         [p,r_indices] = findpeaks(y1diff,'MinPeakHeight',0.4,'MinPeakDistance',60);
